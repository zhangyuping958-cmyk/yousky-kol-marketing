
## 项目工具

- `pptx2text.sh` — 提取 .pptx 文件全部幻灯片文字：`bash pptx2text.sh <文件路径>`

## Yousky 产品体系与 Means-Ends Chain

> 数据来源：`_business-operations/product info/泳池清洁产品介绍-Means-ends Chain.xlsx`

### 产品矩阵总览

| 系列 | 产品 | 定位 | MSRP | MAP | 状态 |
|------|------|------|------|-----|------|
| 水面机器人 | K10 | 中大型地下泳池水面清洁，清洁+净化二合一 | $389.99 | $319.99 | 2026/4 上市 |
| 水面机器人 | Pura V12 | K10 姊妹款，APP 控制+寻阳充电 | TBD | TBD | 规划中 |
| 框架泳池水面机 | S10 基础版 | 地上框架/充气泳池，固定安装式 | $139.99 | $99.99 | 在售 |
| 框架泳池水面机 | S10 太阳能版 | S10 + 太阳能 + 氛围灯 | $199.99 | $149.99 | 在售 |
| 框架泳池水面机 | S20 | 框架泳池，双电机，太阳能辅助 | TBD | TBD | 规划中 |
| 池底机器人 | C20 | 中大型地下泳池池底/池壁/水线，智能路径规划 | $589.99 | $479.99 | 2026/10 上市 |
| 大手持 | H10 | 池底手持清洁，18.5GPM | $134.99 | $90.73 | 在售 |
| 大手持 | H11 | H10 升级：60min 续航，3200mAh | $139.99 | $89-$109 | 即将上市 |
| 大手持 | H11 Pro | H11 + 75μm 超细过滤 | $139.99 | $89-$109 | 即将上市 |
| 大手持 | H20 | 30GPM 大吸力，兼容深水+浅水 | $229.99 | $169 | 规划中 |
| 落叶款 | L10 | 池底/水面落叶，83GPM，180min，24.9L | $199.99 | $129.99 | 在售 |
| 落叶款 | L10 Mini | L10 轻量版，1.29kg | TBD | TBD | 2026/6 上市 |
| 落叶款 | L11 | L10 + 75μm 超细过滤 | $199.99 | $129.99 | 规划中 |
| 小手持 | T10 | SPA/浴缸/小泳池，入水自动开关，120min | $139.99 | $59.99 | 规划中 |
| 小手持 | T11 | T10 升级，90min，多刷头 | $139.99 | $64.99 | 规划中 |
| 配件 | 伸缩杆套装 | 3.5ft/12ft/15ft，通用接口，铝合金 | - | - | 在售 |

### 上市时间轴

```
已在售   H10 / L10 / S10基础版 / S10太阳能版 / 伸缩杆
2026/4   K10（水面机器人旗舰）
2026/6   L10 Mini（落叶轻量版）
2026/10  C20（池底机器人旗舰）
待定     H11 / H11 Pro / H20 / T10 / T11 / L11 / S20 / Pura V12
```

### 渠道与市场
- 主渠道：Amazon + 独立站 .com；市场 US/EU/AU
- 对标竞品：Betta SE（水面机）/ AIPER（池底机）/ Dolphin
- 差异化：高性价比 + 清洁/净化二合一 + 太阳能自主续航

### Means-Ends Chain 摘要（Technology → Feature → Benefit → Value）

**K10 水面机器人 — 终极价值：彻底的自由**
独立氯片仓(移动加药→水质健康→消除隐形焦虑) → 太阳能+25h续航(24/7清洁→零干预→自给自足) → 三无刷电机(2.2㎡/min→效率翻倍→泳池待客状态) → 5L大垃圾篮(海量收纳→极简维护→懒人体验) → 180μm过滤(拦截花粉虫尸→水晶水质→家人健康) → 超声波避障(非接触转向→不磕碰→安静) → 被困自救(智能脱困→无需救援→信赖感) → 433MHz遥控(20m穿透→定点清洁→互动乐趣) → 3.5h快充(极速补能→派对前复活→掌控感) → 航海级耐候(抗UV+耐盐氯→不晒黄不生锈→拒绝塑料玩具)

**S10 框架泳池水面机 — 终极价值：解放框架泳池业主**
2400GPH(吸力3倍→防沉底→水面如镜) → 独立电机(摆脱水泵→游泳不绊脚→保护主泵资产) → 快装支架(无损安装→不钻孔→保护投资) → 3L抽拉篮(一键快取→低频维护→优雅体面) → 太阳能版+4.5h(光能增程→覆盖500sqft→掌控感) → 氛围灯(多彩光效→派对焦点→社交赞赏)

**C20 池底机器人 — 终极价值：智慧与高效的清洁管家**
4800GPH(行业领先→一遍过→极致性能快感) → 弓字形路径(陀螺仪导航→不漏扫→聪明的管家) → 3μm过滤(拦截绿藻细菌→水质透亮→敏感肌可游) → 200min续航(覆盖200㎡+→不趴窝→任务100%完成) → 双履带驱动(坦克抓地力→爬墙稳→可靠性) → 1.3㎡/min(比竞品快30%→缩短等待→把时间还给游泳) → 三电机系统(驱动吸力分离→贴壁稳→极致掌控) → 集成防水+多层防御(滴水不漏→极低故障率→值得托付)

**H10/H11 大手持 — 终极价值：轻量级定点清除**
18.5GPM(轻量级→定点清除→像散步一样简单) → 180μm+75μm双滤袋(按需选配→无细灰回流→健康品质) → 20"-69"伸缩杆(1.75m→岸上清洁不湿身→舒适优雅) → H11: 60min续航(3200mAh→完整清洁不中断→可靠连贯)

**H20 大手持旗舰 — 终极价值：全场景清洁全能掌控**
30GPM(泥沙克星→一遍吸净→效率成就感) → 兼容深水+浅水(2m池底↔12cm台阶→一套顶多套→全能感) → 75μm双层(微米拦截→池底丝滑→完美主义健康) → 可视化面板(电量/模式/故障→消除焦虑→信息透明) → 2.3m杆+双刷头(超长触达→死角零容忍→彻底掌控) → Flash-Swap快拆(一键按压→湿手零门槛→体贴关怀) → 底座支架(站立充电+收纳→空间整洁→秩序感)

**L10/L11 落叶款 — 终极价值：泳池落叶收割机**
83GPM(湿重落叶一扫光→一遍吸净→效率成就) → 24.9L巨量袋(行业领先→一次清全池→从容) → 180min续航(3h连续→多次免充→忠实伙伴) → 兼容水面+水底(一机两用→无需额外工具→全能掌控)

**T10/T11 小手持 — 终极价值：SPA/浴缸专属精致清洁**
入水检测自动开关(入水启动离水停止→无需按键→直觉省心) → 90-120min续航(随取随用→多日碎片化→自由) → 多刷头(三角/滚轮/加长→异形空间零容忍→极致洁净) → 41°F-122°F宽温域(耐热密封→40°C+热水稳定→呵护享受)

### 产品参数速查

| 参数 | H10 | H11 | H11 Pro | H20 | L10 | L11 | T10 | T11 |
|------|-----|-----|---------|-----|-----|-----|-----|-----|
| 吸力(GPM) | 18.5 | 18.5 | 18.5 | 30 | 83 | 83 | TBD | TBD |
| 续航(min) | 35 | 60 | 60 | 60 | 180 | 180 | 120 | 90 |
| 电池(mAh) | 2200 | 3200 | 3200 | TBD | 2200 | 2200 | 2200 | 2200 |
| 充电(h) | 3 | 3 | 3 | 3.5 | 3 | 3 | 3 | 3 |
| 容量(L) | 2.1 | 2.1 | 2.1 | 1.5 | 24.9 | 24.9 | 0.25 | 0.25 |
| 过滤(μm) | 180 | 180 | 180/75 | 180/75 | 180 | 180/75 | 180 | 180 |
| 重量(kg) | 3.2 | 3.2 | 3.28 | TBD | 1.88 | 1.89 | 1.25 | 1.77 |
| MSRP($) | 134.99 | 139.99 | 139.99 | 229.99 | 199.99 | 199.99 | 139.99 | 139.99 |
| MAP($) | 90.73 | 89-109 | 89-109 | 169 | 129.99 | 129.99 | 59.99 | 64.99 |

## Yousky 知识库与资料索引

### 飞书知识库（需登录）
| 资料 | 飞书链接 |
|------|----------|
| 品牌规划 — 品牌故事、使命、愿景、价值观 | https://my.feishu.cn/wiki/UtpgwTimviNe0ekexjfcf7Man23 |
| KOL营销数据看板 | https://my.feishu.cn/wiki/DoLowhubYiDlbEk49kbclJ4En5f |
| 6月邮件营销日历 | https://my.feishu.cn/wiki/KLtGwCrMKipxOqkJKsdcPr2YnDE?sheet=P3g83a |

### 本地资料文件
| 资料 | 路径 |
|------|------|
| 6-8月三渠道备货计划 (20页HTML) | `_business-operations/inventory-slides-html/viewer-zh.html` |
| 备货计划销售预测 (Excel) | `_business-operations/sales-forecast/` |
| 3月工作汇报 (59页PDF) | `_business-operations/monthly review/yousky3月份工作汇报.pdf` |
| 4月工作汇报 (60页PDF) | `_business-operations/monthly review/Yousky4月份工作汇报.pdf` |
| 3月复盘诊断 (15页HTML) | `_marketing-research/yousky-march-review.html` |
| 4月复盘 & 5月战略 (12页HTML) | `_hr-recruitment/marketing-org/april-review-zh.html` |
| 组织架构图 (HTML) | `_hr-recruitment/marketing-org/org-chart-zh.html` |
| 组织架构详情 (8页HTML) | `_hr-recruitment/marketing-org/viewer-zh.html` |
| KOL营销资料 (PDF/XLSX等) | `/Users/qisanjiudekeaiduo/Downloads/坎贝尔/KOL营销/` |
| 红人回复率截图 | `_marketing-research/红人回复率 09.52.43.png` |

## Yousky 月度运营数据

> 数据来源：`_business-operations/monthly review/` 下3月/4月工作汇报PDF

### 团队架构与区分（重要）
- **KOL团队**：独立团队，负责 **YouTube + Instagram** 红人建联（不做TikTok）
- **BD团队**：归属 **TK团队**，专门做 **TikTok** 达人建联
- 两个团队人员、渠道、数据各自独立，不可混用

### 3月数据
| 指标 | 数值 | 来源 |
|------|------|------|
| 销售达成率 | <70% | 3月复盘HTML |
| KOL合作总数 | 12个 | 3月复盘HTML |
| 置换合作 | 4个 | 3月复盘HTML |
| KOL回复率 | ~10%（竞品~20%） | 3月复盘HTML |
| 社媒引流 | 4平台·24帖·6200曝光·4个UV | 3月复盘HTML |

### 4月数据 — KOL团队（YouTube/IG）
| 指标 | 数值 | 来源 |
|------|------|------|
| 寄样 | **13台**（H11:8 / H10:3 / T10:1 / L10:1） | PDF Page 51 |
| 合作费用 | $3,012 + $512物流 | PDF Page 51 |
| 上线视频 | **7条**（H10:4 / H11:3） | PDF Page 53 |
| 视频总曝光 | 323,500 | PDF Page 53 |
| UV | 86 | PDF Page 53 |
| CTR | 0.03% | PDF Page 53 |
| 合作达人 | 13个（持平3月） | PDF Page 51 |
| 付费占比 | 69%（置换31%） | PDF Page 51 |
| 团队人数 | 3人→2人→月底仅剩1人（宝嘉离职） | PDF Page 51 |
| KOL带货转化 | 0单 | 4月复盘HTML |

### 4月数据 — BD/TK团队
| 指标 | 数值 | 来源 |
|------|------|------|
| 达人寄样 | **39台**（H11泳池线为主） | PDF Page 40 / 4月复盘HTML |
| 达人曝光 | 7万+ | 4月复盘HTML |
| TK店铺销量 | 91台（仅10天数据） | 4月复盘HTML |
| GMV Max CPS | $15-20 | 4月复盘HTML |

### 4月数据 — 全渠道
| 指标 | 数值 | 来源 |
|------|------|------|
| 4月销量环比增长 | +33% | 4月复盘HTML |
| 落叶款退货率 | 19%（3月11%→4月19%） | 4月复盘HTML |
| H11问题 | 手柄卡扣返工，新品无法销售 | 4月复盘HTML |

### KOL回复率明细（三专员）
| 专员 | 建联数 | 回复数 | 回复率 |
|------|--------|--------|--------|
| PR | 767 | 79 | **10.3%** |
| FMEN | 1145 | 105 | **9.2%** |
| 3A PRE | 1026 | 53 | **5.2%** |
| **合计** | **2938** | **237** | **8.1%** |

### 4月核心问题摘要
- KOL团队人手崩溃：1人覆盖YouTube+IG全链条，无法支撑新品推广
- TK内容引擎未启动：团队5月才搭完
- 产品端：落叶款退货率19%、H11手柄返工、小手持差评炸店
- 全渠道定价混乱：TK $99 / Amazon $139 / DTC $179，差价$80
- 5月战略：私域引擎、定价统一、KOL团队重建 |

### Memory 系统
用户偏好和项目上下文存储在 `.claude/projects/.../memory/MEMORY.md`，启动新对话时自动加载。

<!-- pneuma:start -->
## Pneuma Slide Mode

You are running inside **Pneuma**, a co-creation workspace where you and the user build content together — you edit files, the user sees live results in a browser preview panel.

This is **Slide Mode**: HTML presentation creation with live fixed-viewport preview.

For design workflow, height calculation rules, layout patterns, and quality checklist, consult the `pneuma-slide` skill. Slides have no scroll — getting the layout right requires the skill's guidance.

### Architecture
- `slides/*.html` — HTML fragments per slide (no `<html>`/`<body>` tags)
- `manifest.json` — Slide ordering (always update when adding/removing slides)
- `theme.css` — Shared CSS theme via custom properties
- Canvas: 1280×720px fixed viewport — content beyond this is invisible
- **Content sets**: Each top-level directory (e.g. `en-dark/`, `my-deck/`) is a switchable content set with its own slides, manifest, and theme

### Core Rules
- Content must fit within 1280×720px — overflow is the #1 quality issue (no scroll)
- No CSS animations — they break snapshot-based export and print
- **New task → new content set**: When the user asks for a completely new presentation, create a new top-level directory (content set) rather than overwriting existing content — this preserves seed templates and prior work
- **Importing external content → new content set**: When the user provides original content (uploaded files, pasted slides, or a URL), always create a new content set for it. Place imported files inside the new directory with a proper `manifest.json` and `theme.css`. This ensures seed templates are preserved and all built-in features (set switching, comparison, export) work correctly.
- For new decks: design outline first → theme → scaffold → fill content
- Do not ask for confirmation on simple edits — just do them

<!-- pneuma:end -->

<!-- pneuma:viewer-api:start -->
## Viewer API

### Viewer Context

Each user message may be prefixed with a `<viewer-context>` block.
It describes what the user is currently seeing — the active file, viewport position, and selected elements.
Use this to resolve references like "this page", "here", "this section" in user messages.

### User Actions

Messages may include a `<user-actions>` block listing significant actions
the user performed in the viewer since the last message.
Use this to understand workspace state changes that happened outside of your edits.

### Workspace
- Type: manifest (ordered, multi-file, active file tracking)
- Index file: manifest.json

### Content Sets
This workspace may contain multiple content sets as top-level directories (e.g. en-dark/, ja-light/).
The `<viewer-context>` includes a `content-set` attribute. File paths include the content set prefix.
Always edit files within the active content set's directory unless asked to work across content sets.

### Actions

The viewer supports these operations. Invoke via Bash:
`curl -s -X POST $PNEUMA_API/api/viewer/action -H 'Content-Type: application/json' -d '{"actionId":"<id>","params":{...}}'`

| Action | Description | Params |
|--------|-------------|--------|
| `navigate-to` | Navigate to a specific slide | file: string |

### Scaffold

Initialize workspace with slide scaffolding from a structure spec. When creating a new theme/deck, pass contentSet to avoid overwriting the active content set. **Requires user confirmation in browser.**

Invoke via the viewer action API:
`curl -s -X POST $PNEUMA_API/api/viewer/action -H 'Content-Type: application/json' -d '{"actionId":"scaffold","params":{...}}'`

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `title` | string | yes | Presentation title |
| `slides` | string | yes | JSON array of {title, subtitle?} |
| `contentSet` | string | no | Target content set name (e.g. 'my-theme'). If omitted, overwrites the active content set. |

Clears: `slides/*.html`, `manifest.json`

### Locator Cards

You may embed clickable navigation cards in your messages using this tag:
`<viewer-locator label="Display Label" data='{"key":"value"}' />`

After creating or editing slides, embed locator cards so the user can jump to them. Navigate by file: `data='{"file":"slides/slide-03.html"}'`. Navigate by number: `data='{"index":3}'`. Switch content set: `data='{"contentSet":"deck-2"}'`. Switch content set and slide: `data='{"contentSet":"deck-2","index":1}'`.

When the user clicks a locator card, the viewer navigates to that location.

**Always** embed locator cards at the end of your response when you create or edit content. The user may have navigated away while you were working — locators let them jump directly to what changed.

### Native Desktop APIs

The runtime provides native desktop capabilities via `/api/native/`. Available when running inside the Pneuma desktop app.

**Discovery:** `curl -s $PNEUMA_API/api/native` — returns `{ available: true, capabilities: { module: [methods...] } }` or `{ available: false }`.
Always check this first to see what's available — the capability list is dynamic and auto-generated from Electron modules.

**Invoke:** `curl -s -X POST $PNEUMA_API/api/native/<module>/<method> -H 'Content-Type: application/json' -d '[...args]'`
Returns `{ ok: true, result: ... }` or `{ ok: false, error: "..." }`.

**Common modules:** `clipboard` (readText, writeText, readImage→base64, writeImage←base64, ...), `shell` (openPath, openExternal, ...), `app` (getVersion, getPath, ...), `system` (platform, cpus, totalMemory, hostname, ...), `screen`, `nativeTheme`, `notification` (show, isSupported), `window` (minimize, maximize, setAlwaysOnTop, getBounds, ...)

<!-- pneuma:viewer-api:end -->

<!-- pneuma:skills:start -->
## Available Skills

The following skills are installed and available for use:

- **pneuma-preferences** — Persistent user preference memory. Consult BEFORE making design, style, or aesthetic decisions in any mode. Also use when starting creative work or when the user corrects your choices.

Use `/<skill-name>` to invoke these skills.
<!-- pneuma:skills:end -->
